...
detector_r = 10 # Detector radius [m]
pmt_r = 6.7 # PMT support structure radius [m]
...
hs_key = "heysham_signal"
hb_key = "heysham_background"
...
class SingleEvent:
    ...
    def add(self, mc_x, mc_y, mc_z, reco_x, reco_y, reco_z, mc_energy, inner_hits, veto_hits, n9):
    ...
    def efficiency(self, wall_cut, energy_cut, reco=True):
        ...
        if reco:
            eff = np.count_nonzero((self.values['reco_dwall'] > wall_cut)
                                    & (self.values['n9'] != -99999)
                                    & (self.values['n9'] > energy_cut)
                                    & (self.values['veto_hits'] < 4))
        eff = eff/len(self.values['mc_x'])
        return eff
    ...
    def selected(self, wall_cut, energy_cut, index, reco=True):
        ...
        if not reco and ((self.values['dwall'][index] > wall_cut)
            ...
            sel = True
        elif reco and ((self.values['reco_dwall'][index] > wall_cut)
                       & (self.values['n9'][index] != -99999)
...
def calc_signal_rate(singles, wall_p_cut, energy_p_cut, wall_n_cut,
    ...
    # TODO using fairly simple model for neutron interaction time and distance
    # would be better to just draw from a high stats histogram
    sep_time = np.random.exponential(200, size=num_samples) # [mus]
    ...
    sep_dist = np.random.gumbel(8, size=num_samples) # [cm]
...
def sensitivity(singles, wall_p_cut, energy_p_cut, wall_n_cut, energy_n_cut,
                dt_cut, ds_cut, reco=True, verbose=False):
...
def dwell_time_scaling(singles, wall_p_cut, energy_p_cut, wall_n_cut,
    ...
    background_rate = signal_rate*rates[hb_key]/rates[p_key] + accidental_rate
    # Scale Hartlepool to expected Heysham signal
    signal_rate = signal_rate*rates[hs_key]/rates[p_key]
    if verbose:
...
root_file = ROOT.TFile.Open("wbls3pct_reco.root")
...
print('dwell time (scaling) = ',dwell_time_scaling(singles, 2, 27, 2, 15, 400, 200, 3, True, True),' days')
print('dwell time (spectra) = ',dwell_time_heysham(singles, 2, 27, 2, 15, 400, 200, 3, True, True),' days')
